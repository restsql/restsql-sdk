...
CREATE TABLE film_comment ( film_id SMALLINT NOT NULL, the_comment VARCHAR(128), foreign key (film_id) references film (film_id) on delete cascade);
...
CREATE TABLE datetime ( id SMALLINT NOT NULL, the_time timestamp , the_timestamp timestamp , the_date date , the_datetime date );
