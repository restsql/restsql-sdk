...
-- 		1) changed some integer types to SMALLINT to be compatible with MySQL's sakila database
--		2) changed language.name to be varchar so it is not right padded
--
...
CREATE TABLE actor (
    actor_id smallint DEFAULT nextval('actor_actor_id_seq'::regclass) NOT NULL,
...
CREATE TABLE film (
    film_id smallint DEFAULT nextval('film_film_id_seq'::regclass) NOT NULL,
